...
%% simulate GP;
nb_MC = 500; % Number of profiles to be averaged
...
for i = 1:nb_MC
    ...
    GP_seq2 = sample_GP(x, sigma, xc, nb_GP);
    ...
    u = conj(U_L(i,:)') .* mirror;
    U_0_rand(i,:) = split_step_fourier_method(round(L/h), -1, 0, u, h, k, f, GP_seq);
    
    % Go reverse in homogeneous medium
    u = conj(U_L(i,:)') .* mirror;
    U_0_homo(i,:) = split_step_fourier_method(round(L/h), -1, 0, u, h, k, f);
end
U_L = mean(U_L, 1)';
...
figure(1); plot(x, real(U_0_homo), x, imag(mean_wave_0_homo));
figure(2); plot(x, imag(U_0_homo), x, real(mean_wave_0_homo));
